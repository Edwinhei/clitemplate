...
export const gate: Gate = {
  ...
  run(root) {
    ...
    for (const pkg of listPackages(root)) {
      ...
      if (!m.description) {
        v.push({ file, message: '缺 description', fix: '写一句话说清它在系统里占什么位置，不要写「XX 模块」' })
      }
      if (!m.scripts?.typecheck) {
        v.push({ file, message: '缺 typecheck 脚本', fix: '"scripts": { "typecheck": "tsc --noEmit" }' })
      }

      if (pkg.kind === 'package') {
        if (!m.exports) {
          v.push({ file, message: '能力包缺 exports', fix: '"exports": { ".": "./src/index.ts", "./package.json": "./package.json" }' })
        ...
        if (m.bin) {
          v.push({ file, message: '能力包不该有 bin', fix: 'bin 是应用的属性。有 bin 说明它其实是个 app，该放进 apps/' })
        }
      }

      if (pkg.kind === 'app' && m.exports) {
        v.push({ file, message: 'app 不该有 exports', fix: 'app 处在依赖图顶端，没有人 import 它。它靠 bin 交付，不靠被导入' })
      }
    }
    return v
  },
